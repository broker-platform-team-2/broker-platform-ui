    ...
    <div style={{
      ...
      <header style={{
        ...
        <nav style={{ display: 'flex', gap: 28, fontSize: 13.5 }}>
          {['Markets', 'Pricing', 'Impact', 'Learn', 'About'].map(l => (
            <a key={l} href="#" style={{ color: D.ink70, textDecoration: 'none', fontWeight: 500 }}>{l}</a>
          ))}
        </nav>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative' }}>
          <div style={{
            ...
            background: D.sageBg, border: `1px solid ${D.sage}33`, borderRadius: 999,
            fontSize: 12, color: D.sage, fontWeight: 600, marginBottom: 24,
          }}>
            <span style={{ width: 6, height: 6, borderRadius: 3, background: D.sage, boxShadow: `0 0 8px ${D.sage}` }}/>
            Now live · 9 listed equities · €0 commission
          </div>
          <h1 style={{
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
            ...
            <button style={{
              background: 'transparent', border: `1px solid ${D.hairline2}`, color: D.ink,
              padding: '14px 22px', borderRadius: 11, fontFamily: FONT_HEAD, fontWeight: 600, fontSize: 14, cursor: 'pointer',
            }}>Watch 90s demo</button>
          </div>
          <div style={{ display: 'flex', gap: 28, marginTop: 36, fontSize: 12.5, color: D.ink50 }}>
            <span>★ 4.9 · App Store</span>
            <span>● 24/7 simulation tape</span>
      ...
      <section style={{ padding: '0 48px 60px' }}>
        <div style={{
          background: D.surface, borderRadius: 18, border: `1px solid ${D.hairline}`,
          padding: '32px 40px',
          display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 32,
        }}>
          {[
            ['€2.4M', 'Daily volume on Wakibi Exchange'],
            ['18,420', 'Active traders'],
            ['€124k', 'Microloans funded YTD'],
            ['70+', 'Countries reached'],
          ].map(([v, l]) => (
            <div key={l}>
              <div style={{ fontFamily: FONT_HEAD, fontWeight: 700, fontSize: 36, color: D.sage, letterSpacing: '-0.02em', lineHeight: 1 }}>{v}</div>
              <div style={{ fontSize: 13, color: D.ink70, marginTop: 8 }}>{l}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '0 48px 80px' }}>
        <div style={{
          ...
          background: `linear-gradient(135deg, ${D.plum} 0%, ${D.plumDeep} 100%)`,
          position: 'relative', overflow: 'hidden',
          display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, alignItems: 'center',
        }}>
          <div style={{
            position: 'absolute', right: -100, bottom: -200, width: 500, height: 500,
            borderRadius: '50%', background: `radial-gradient(circle, ${D.spring}22 0%, transparent 60%)`,
          }}/>
          <div style={{ position: 'relative' }}>
            <div style={{ fontSize: 12, color: D.spring, textTransform: 'uppercase', letterSpacing: 1, fontWeight: 700, marginBottom: 14 }}>The Wakibi loop</div>
            <h2 style={{ fontFamily: FONT_HEAD, fontWeight: 700, fontSize: 38, color: '#fff', letterSpacing: '-0.02em', margin: 0, lineHeight: 1.1 }}>
              Every fee becomes<br/>a small zetje for someone.
            </h2>
            <p style={{ fontSize: 15, color: 'rgba(255,255,255,0.8)', lineHeight: 1.6, marginTop: 20, maxWidth: 460 }}>
              You pay a fee on a trade. We bundle those fees into microloans and route them to entrepreneurs in 70+ countries. They build something. They pay it back. We loan it again. The loop never stops.
            </p>
            <button style={{
              marginTop: 28,
              background: D.spring, color: D.plumDeep, border: 'none',
              padding: '12px 22px', borderRadius: 10,
              fontFamily: FONT_HEAD, fontWeight: 700, fontSize: 14, cursor: 'pointer',
            }}>See where fees go →</button>
          </div>
          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 12 }}>
            {[
              { country: 'Kenya', name: 'Amina · Maize farmer',  amount: '€420', pct: 78 },
              { country: 'Peru',  name: 'Diego · Bike repair',   amount: '€280', pct: 45 },
              { country: 'Vietnam', name: 'Linh · Tailor co-op', amount: '€640', pct: 92 },
            ].map(l => (
              <div key={l.name} style={{
                ...
                <div>
                  <div style={{ fontSize: 13, color: '#fff', fontWeight: 600 }}>{l.name}</div>
                  <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.6)', marginTop: 2 }}>{l.country} · {l.pct}% repaid</div>
                  <div style={{ height: 4, background: 'rgba(255,255,255,0.1)', borderRadius: 2, marginTop: 6, overflow: 'hidden' }}>
                    <div style={{ width: l.pct + '%', height: '100%', background: D.spring }}/>
                  </div>
                </div>
                <div style={{ fontFamily: FONT_HEAD, fontWeight: 700, color: D.spring, fontSize: 16 }}>{l.amount}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <footer style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, auto)', gap: 60 }}>
          {[
            ['Product', ['Markets', 'Pricing', 'Mobile app', 'API']],
            ['Company', ['About', 'Impact report', 'Careers', 'Press']],
            ['Resources', ['Docs', 'Help center', 'Community', 'Status']],
            ['Legal', ['Terms', 'Privacy', 'Disclosures', 'Security']],
          ].map(([h, items]) => (
            <div key={h}>
              <div style={{ color: D.ink, fontWeight: 600, marginBottom: 10, fontSize: 12.5 }}>{h}</div>
