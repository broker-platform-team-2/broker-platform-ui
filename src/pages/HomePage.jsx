...
export default function HomePage() {
    ...
    (async () => {
      try {
        ...
        if (h.status === 'fulfilled') {
          ...
          // Fetch history for holdings to make the chart "actually work"
          const tickers = Array.from(new Set(holds.map(r => r.instrumentId || r.ticker).filter(Boolean)));
          const hists = await Promise.allSettled(tickers.map(t => getStockHistory(t, '1M')));
          const newMap = {};
          hists.forEach((res, i) => {
            if (res.status === 'fulfilled') newMap[tickers[i]] = res.value;
          });
  ...
  return (
    <AppShell title="Home"
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 18, marginBottom: 18, alignItems: 'stretch' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          <Card padding={22} style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <div style={{ width: '80%', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
