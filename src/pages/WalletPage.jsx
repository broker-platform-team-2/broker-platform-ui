...
export default function WalletPage() {
  const [active, setActive] = useState('acc_eur');
  const [tab, setTab] = useState('all');
  const [eurAvailable, setEurAvailable] = useState(null);

  useEffect(() => {
    getBalance().then(b => setEurAvailable(b?.available)).catch(() => {});
  }, []);

  const account = ACCOUNTS.find(a => a.id === active);
  const headerBalance = eurAvailable ?? ACCOUNTS.find(a => a.currency === 'EUR').balance;
  const movements = (account.currency === 'EUR' ? MOVEMENTS_EUR : MOVEMENTS_RON).filter(m => {
  ...
  return (
    <AppShell title="Wallet" subtitle="Accounts, deposits & transfers" balance={headerBalance}>
      {/* Account cards */}
        ...
        {ACCOUNTS.map(a => {
          ...
          return (
            <button key={a.id} onClick={() => setActive(a.id)} style={{
              ...
              background: sel
                ? `linear-gradient(135deg, ${isEur ? D.sage : D.orchid} 0%, ${isEur ? D.teal : D.plum} 100%)`
                : D.surface,
              border: `1px solid ${sel ? 'transparent' : D.hairline}`,
              borderRadius: 16, padding: 22,
              color: sel ? (isEur ? D.plumDeep : '#fff') : D.ink,
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 22 }}>
                <div>
                  <div style={{ fontSize: 11, opacity: 0.7, textTransform: 'uppercase', letterSpacing: 0.6, fontWeight: 600 }}>
                    {isEur ? 'Primary · Euro' : 'Secondary · Romanian leu'}
                  </div>
                  <div style={{ fontFamily: FONT_HEAD, fontWeight: 600, fontSize: 14, marginTop: 4 }}>NL98 WAKB ···· {isEur ? '4421' : '7745'}</div>
                </div>
                <div style={{
              ...
              <div style={{ fontFamily: FONT_HEAD, fontWeight: 700, fontSize: 30, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums' }}>
                {a.currency === 'EUR' ? '€' : 'lei '}{a.balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
              {a.frozen > 0 && (
                <div style={{ fontSize: 11.5, opacity: 0.78, marginTop: 6 }}>
                  {a.currency === 'EUR' ? '€' : 'lei '}{a.frozen.toFixed(2)} frozen in open orders
                </div>
              )}
            </button>
          );
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 18 }}>
        {[
          { id: 'deposit',  label: 'Deposit',     icon: '↓', primary: true },
          { id: 'withdraw', label: 'Withdraw',    icon: '↑' },
          { id: 'transfer', label: 'Transfer',    icon: '⇌' },
          { id: 'card',     label: 'Wakibi card', icon: '◰' },
        ].map(a => (
        ...
        <Card padding={22}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: 12, color: D.buy, fontWeight: 600 }}>+€7,920.55 this month</div>
            </div>
          </div>
      ...
      <Card padding={0}>
        <div style={{ padding: '18px 22px', borderBottom: `1px solid ${D.hairline}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: D.ink50, marginTop: 2 }}>{movements.length} entries · {account.currency}</div>
          </div>
          ...
          return (
            <div key={m.id} style={{
              ...
              <div style={{ textAlign: 'right', fontFamily: FONT_HEAD, fontWeight: 700, fontSize: 15,
                ...
                {isIn ? '+' : '−'}{sym}{Math.abs(m.amount).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </div>
            </div>
          );
        })}
